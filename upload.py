...
def rebuild_index():
    ...
    reports = sorted(
        [f for f in REPO_DIR.glob("*market*.html")
         if f.name != "index.html"
         and re.search(r'\d{8}', f.name)  # 날짜 있는 파일만
         and not f.name.startswith('global_market_weekly')
         and not f.name.startswith('us_market_weekly')],
